...
from torchvision.transforms import functinoal as VF, InterpolationMode
...
def get_spike_counter(ckpt_path: str):
    config = SegformerConfig.from_pretrained(SEGFORMER_MODEL_NAME)
    config.num_labels = 1

    base_model = SegformerForSemanticSegmentation.from_pretrained(config=config)
