...
class StudPipelineOptions(BaseModel):
    ...
    nms_iou_threshold: float = Field(0.15, description="Non-maximum suppression (NMS) IoU threshold for merging overlapping stud detections. Lower values mean higher merging rate", ge=0, le=1)
    confidence_threshold: float = Field(0.25, description="Confidence threshold for stud detection. Lower values mean higher sensitivity", ge=0, le=1)
...
class SidewallUnwrapperOptions(BaseModel):
    polar_unwrap_size: Tuple[int, int] = Field((1000, 2500), description="Detected sidewall is flattened to an image of this resolution and passed to the OCR. Higher values mean more detailed image (higher OCR quality), but also higher quota consumption and processing time")


class OCROptions(BaseModel):
    model_name: str = Field("qwen/qwen3-vl-30b-a3b-instruct", description="Name of a VLM model to use for OCR from openrouter.ai. Insert any valid VLM model from https://openrouter.ai/models")
    providers_list: List[str] = Field(["parasail"], description="List of providers to use for OCR. Insert any valid provider from https://openrouter.ai/providers")
...
class IndexOptions(BaseModel):
    ...
    max_distinct_matches: int = Field(3, description="Maximum number of distinct matches to consider for each detected string. Lower values mean more diversity in the results", ge=0)
    brand_model_match_bonus: float = Field(0.1, description="How much score is added to pairs of model and brand matches in the database. Higher values mean more likely to return existing brand-model pairs from the database", ge=0)
