...
class OnnxSegmentator:
    def __init__(self, onnx_path: str, resize_shape: Tuple[int, int], resize_mask_shape: Optional[Tuple[int, int]] = None):
    ...
    def forward(self, image: np.ndarray, threshold: Optional[float] = None):
        ...
        resized_image = cv2.resize(image, self.resize_shape, interpolation=cv2.INTER_LINEAR).transpose(2, 0, 1)[None].astype(np.float32) / 255
