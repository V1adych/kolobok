...
class Args:
    ...
    ckpt_dir: str = "classification_checkpoints"
    no_aug: bool = False
    ...
    def __init__(self, roots: List[str], transform: Optional[transforms.Compose]):
        ...
        for root in map(Path, roots):
            ...
            for _, row in labels.iterrows():
                img_path = images_root / row["image_name"]
...
class SpikeClassificationModule(pl.LightningModule):
    def __init__(self, args: Args, total_steps: int):
        super().__init__()
        self.save_hyperparameters(vars(args))
        self.args = args
        self.total_steps = total_steps

        self.model = get_model(args.model_name, num_classes=args.num_classes)
    ...
    def training_step(self, batch, batch_idx):
        ...
        acc = self._metrics(preds, labels)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), lr=self.args.max_lr)
...
def main():
    ...
    train_ds = SpikeClassificationDataset(args.train_roots, transform=train_tf)
    val_ds = SpikeClassificationDataset(args.val_roots, transform=val_tf)
    ...
    module = SpikeClassificationModule(
        args,
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        filename="model-{epoch:03d}-{val_acc:.5f}",
        monitor="val_acc",
        mode="max",
        save_top_k=10,
        save_last=True,
